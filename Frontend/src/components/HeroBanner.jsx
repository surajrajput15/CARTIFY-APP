...
const HeroBanner = () => {
  ...
  return (
    <section
      ...
      className="relative overflow-hidden rounded-3xl shadow-xl mb-10 border border-teal-500/20"
      aria-label="Feature highlights carousel"
      aria-roledescription="carousel"
    >
      {/* Animated rotating gradient background (CSS keyframe) */}
      <div
        className={`absolute inset-0 bg-gradient-to-r ${slide.gradient} hero-grad-animate transition-colors duration-700`}
        aria-hidden="true"
      ></div>
      ...
      <div className="absolute inset-0 pointer-events-none z-0" aria-hidden="true">
        {slide.floaters.map((floater, index) => {
          ...
          return (
            <span
              key={`${slide.id}-${index}`}
              className={`absolute ${floater.className} ${reducedMotion ? '' : floater.anim} text-4xl sm:text-5xl md:text-6xl select-none drop-shadow-lg`}
      ...
      {/* Keyed by slide id so the staggered entrance re-runs on every change */}
      <div
        key={slide.id}
        className="relative flex flex-col md:flex-row items-center justify-between p-6 sm:p-8 md:p-14 gap-6 md:gap-8"
      >
        {/* Left Side: Text and Button */}
        <div className="text-white z-10 md:w-3/5 text-center md:text-left">
          <div
            className={`inline-flex items-center gap-2 bg-white/10 backdrop-blur-md px-3 py-1.5 rounded-full border ${slide.badgeClass} hero-rise`}
          >
          ...
          <p className="text-teal-100 text-sm sm:text-base md:text-lg max-w-xl font-medium leading-relaxed hero-rise" style={{ animationDelay: '0.55s' }}>
            {slide.id === 'tech' ? (
          ...
          <button
            ...
            className="mt-4 md:mt-6 inline-flex items-center gap-2 bg-white text-teal-700 font-extrabold py-3 px-8 sm:px-10 rounded-xl shadow-lg hover:bg-teal-50 hover:text-teal-800 hover:-translate-y-0.5 hover:shadow-xl transition-all duration-300 active:translate-y-0 cursor-pointer min-h-[44px] hero-bounce-in"
        ...
        <div className="mt-4 md:mt-0 md:w-2/5 flex justify-center z-10 w-full hero-bounce-in" style={{ animationDelay: '0.4s' }}>
          <HeroIllustration className="w-full max-w-xs sm:max-w-sm h-56 sm:h-64 md:h-72" />
        </div>
      </div>

      {/* Controls row: arrows + autoplay toggle + indicators (below content) */}
      <div className="relative z-10 flex items-center justify-between px-6 sm:px-8 pb-4 md:pb-6">
        ...
        <div className="flex items-center gap-2" role="tablist" aria-label="Choose slide">
          {SLIDES.map((s, index) => (
            <button
              ...
              className={`w-2.5 h-2.5 rounded-full transition-all duration-300 cursor-pointer min-h-[36px] min-w-[36px] flex items-center justify-center p-0 ${
                index === activeIndex
                  ? 'bg-white scale-125'
                  : 'bg-white/40 hover:bg-white/70'
              }`}
            >
              <span className={`w-2.5 h-2.5 rounded-full ${index === activeIndex ? 'bg-white' : 'bg-white/40'}`}></span>
            </button>
          ))}
        </div>
